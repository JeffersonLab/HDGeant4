//
// GlueXSensitiveDetectorITOF - class implementation
//
...
GlueXSensitiveDetectorITOF::GlueXSensitiveDetectorITOF(const G4String& name)
 : G4VSensitiveDetector(name),
   fHitsMap(0), fPointsMap(0)
{
   collectionName.insert("ITOFHitsCollection");
   collectionName.insert("ITOFPointsCollection");
   ...
   // the properties of hits in the ITOF, you must delete all old
   ...
   if (instanceCount++ == 0) {
      ...
      if (japp == 0) {   // dummy
         jcalib = 0;
         G4cout << "ITOF: ALL parameters loaded from ccdb" << G4endl;
      }
   }
}

GlueXSensitiveDetectorITOF::GlueXSensitiveDetectorITOF(
                     const GlueXSensitiveDetectorITOF &src)
...
GlueXSensitiveDetectorITOF &GlueXSensitiveDetectorITOF::operator=(const
                                         GlueXSensitiveDetectorITOF &src)
{
   G4AutoLock barrier(&fMutex);
...
void GlueXSensitiveDetectorITOF::Initialize(G4HCofThisEvent* hce)
{
   fHitsMap = new
              GlueXHitsMapITOFhit(SensitiveDetectorName, collectionName[0]);
   fPointsMap = new
              GlueXHitsMapITOFpoint(SensitiveDetectorName, collectionName[1]);
...
G4bool GlueXSensitiveDetectorITOF::ProcessHits(G4Step* step, 
                                                G4TouchableHistory* ROhist)
{
  ...
  if (trackinfo->GetGlueXHistory() == 0 && itrack > 0 && xin.dot(pin) > 0) {
    ...
    GlueXHitITOFpoint* lastPoint = (*fPointsMap)[key - 1];
  ...
  // Post the hit info to the hits map
  
  if (dEsum > 0) {
    int key = GlueXHitITOFhit::GetKey(0);
    GlueXHitITOFhit *counter = (*fHitsMap)[key];
    if (counter == 0) {
      GlueXHitITOFhit newhit(0);
...
void GlueXSensitiveDetectorITOF::EndOfEvent(G4HCofThisEvent*)
{
   std::map<int,GlueXHitITOFhit*> *hits = fHitsMap->GetMap();
   std::map<int,GlueXHitITOFpoint*> *points = fPointsMap->GetMap();
   if (hits->size() == 0 && points->size() == 0)
      return;
   std::map<int,GlueXHitITOFhit*>::iterator siter;
   std::map<int,GlueXHitITOFpoint*>::iterator piter;

   if (verboseLevel > 1) 
     { 
      G4cout << G4endl
             << "--------> Hits Collection: in this event there are "
             << hits->size() << " hits in the ITOF: "
             << G4endl;
      ...
      G4cout << G4endl
             << "--------> Hits Collection: in this event there are "
             << points->size() << " truth points in the ITOF: "
             << G4endl;
   ...
   if (record == 0) {
      G4cerr << "GlueXSensitiveDetectorITOF::EndOfEvent error - "
   ...
   if (hitview.getInnerTOFs().size() == 0)
      hitview.addInnerTOFs();
   hddm_s::InnerTOF &innerTOF = hitview.getInnerTOF();

   // Collect and output the itofTruthHits
   for (siter = hits->begin(); siter != hits->end(); ++siter) {
     std::vector<GlueXHitITOFhit::hitinfo_t> &hits = siter->second->hits;
     hddm_s::ItofCounterList counter = innerTOF.addItofCounters(1);
    
     for (int ih=0; ih < (int)hits.size(); ++ih) {
       // Position at entry to volume
       double x=hits[ih].x_cm;
       double y=hits[ih].y_cm;
       double t=hits[ih].t_ns;
       double dE=hits[ih].dE_GeV;
  
       hddm_s::ItofTruthHitList thit = counter(0).addItofTruthHits(1);
       thit(0).setT(t);
       thit(0).setDE(dE);
       thit(0).setX(x);
       thit(0).setY(y);
   ...
   // Collect and output the itofTruthPoints
   for (piter = points->begin(); piter != points->end(); ++piter) {
      hddm_s::ItofTruthPointList point = innerTOF.addItofTruthPoints(1);
...
int GlueXSensitiveDetectorITOF::GetIdent(std::string div, 
