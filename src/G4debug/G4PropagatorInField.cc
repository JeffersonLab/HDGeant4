...
{  
  ...
  { 
      ...
      // CurrentState is now updated with the final position and velocity
   {
    ...
    std::stringstream sdilog;
    sdilog.precision(9);
    sdilog << "G4PropagatorInField::ComputeStep at AdvanceChordLimited " << do_loop_count
	   << " returns " << s_length_taken
    ...
    ++do_loop_count;
   {
    ...
    std::stringstream sdilog;
    sdilog.precision(9);
    sdilog << "G4PropagatorInField::ComputeStep do_loop_count=" << do_loop_count
           << ", End_PointAndTangent="  << End_PointAndTangent
           << ", OriginalState=" << OriginalState
           << ", CurrentState=" << CurrentState
           << ", StepTaken=" << StepTaken;
    dilog::get(evno.str().c_str()).printf("%s", sdilog.str().c_str());
