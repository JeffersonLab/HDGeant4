                                 ...
                                 G4VPhysicalVolume* currentVolume)
{
...

  {
    ...
    sdilog.precision(9);
    sdilog << "G4PathFinder::ComputeStep entered, "
       ...
       Locate( newPosition, newDirection ); 
  {
    ...
    std::stringstream sdilog;
    sdilog.precision(9);
    sdilog << "G4PathFinder::ComputeStep : Point moved since last step "
           << "by " << std::sqrt(moveLenSq) << " to " << newPosition << ", dir " << newDirection;
    dilog::get(evno.str().c_str()).printf("%s", sdilog.str().c_str());
  }
    }
       ...
       DoNextCurvedStep( currentState, proposedStepLength, currentVolume ); 
   {
    ...
    std::stringstream sdilog;
    sdilog.precision(9);
    sdilog << "G4PathFinder::ComputeStep : back from DoNextCurvedStep with "
           << "fCurrentStepSize[" << navigatorNo << "]=" << fCurrentStepSize[navigatorNo];
    dilog::get(evno.str().c_str()).printf("%s", sdilog.str().c_str());
  }    //--------------
    }else{
       DoNextLinearStep( currentState, proposedStepLength ); 
  {
    ...
    std::stringstream sdilog;
    sdilog.precision(9);
    sdilog << "G4PathFinder::ComputeStep : back from DoNextLinearStep with "
           << "fCurrentStepSize[" << navigatorNo << "]=" << fCurrentStepSize[navigatorNo];
    dilog::get(evno.str().c_str()).printf("%s", sdilog.str().c_str());
  }    //--------------
    }
  ...
  { 
   ...
   GlueXUserEventInformation *eventinfo = (GlueXUserEventInformation*)
                                          event->GetUserInformation();
   evno << "event_" << eventinfo->GetEventNo();
   dilog::get(evno.str().c_str()).printf("G4PathFinder::ComputeStep returns "
		                         "%.9g for Navigator %d, "
					 "Limited step= %.9g",
                                         fCurrentStepSize[navigatorNo],
                                         navigatorNo, limitedStep);
                                      ...
                                      G4double      proposedStepLength )
{
  ...
  if( fVerboseLevel > 1 )
  {
   ...
   GlueXUserEventInformation *eventinfo = (GlueXUserEventInformation*)
                                          event->GetUserInformation();
   evno << "event_" << eventinfo->GetEventNo();
   dilog::get(evno.str().c_str()).printf("G4PathFinder::DoNextLinearStep : "
		                         "initialPosition = (%.9f, %.9f, %.9f), "
		                         "endPosition = (%.9f, %.9f, %.9f)",
                                         initialPosition.x(),
                                         initialPosition.y(),
                                         initialPosition.z(),
                                         endPosition.x(),
                                ...
                                G4VPhysicalVolume* pCurrentPhysicalVolume )
{
...
#endif
  {
    ...
    std::stringstream sdilog;
    sdilog.precision(9);
    sdilog << "G4PathFinder::DoNextCurvedStep entered, "
...
#endif
 {
    ...
    std::stringstream sdilog;
    sdilog.precision(9);
    sdilog << "G4PathFinder::DoNextCurvedStep : "
