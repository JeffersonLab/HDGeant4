//
// GlueXSensitiveDetectorECAL - class implementation
//
...
// Geometry constants for the ECAL
double GlueXSensitiveDetectorECAL::WIDTH_OF_BLOCK = 2.09*cm;
double GlueXSensitiveDetectorECAL::LENGTH_OF_BLOCK = 20.0*cm;
int GlueXSensitiveDetectorECAL::CENTRAL_COLUMN = 24;
int GlueXSensitiveDetectorECAL::CENTRAL_ROW = 24;
...
double GlueXSensitiveDetectorECAL::THRESH_MEV = 5.;

int GlueXSensitiveDetectorECAL::instanceCount = 0;
G4Mutex GlueXSensitiveDetectorECAL::fMutex = G4MUTEX_INITIALIZER;

GlueXSensitiveDetectorECAL::GlueXSensitiveDetectorECAL(const G4String& name)
 : G4VSensitiveDetector(name),
   fBlocksMap(0), fPointsMap(0)
{
   collectionName.insert("ECALBlockHitsCollection");
   collectionName.insert("ECALPointsCollection");
   ...
   // the drift-time properties of hits in the ECAL, you must delete all old
...
GlueXSensitiveDetectorECAL::GlueXSensitiveDetectorECAL(
                     const GlueXSensitiveDetectorECAL &src)
...
GlueXSensitiveDetectorECAL &GlueXSensitiveDetectorECAL::operator=(const
                                         GlueXSensitiveDetectorECAL &src)
{
   G4AutoLock barrier(&fMutex);
...
void GlueXSensitiveDetectorECAL::Initialize(G4HCofThisEvent* hce)
{
   fBlocksMap = new
              GlueXHitsMapECALblock(SensitiveDetectorName, collectionName[0]);
   fPointsMap = new
              GlueXHitsMapECALpoint(SensitiveDetectorName, collectionName[1]);
                                               ...
                                               G4TouchableHistory* ROhist)
{
   ...
   if (dEsum > 0) {
      ...
      int key = GlueXHitECALblock::GetKey(column, row);
      GlueXHitECALblock *block = (*fBlocksMap)[key];
      if (block == 0) {
         GlueXHitECALblock newblock(column, row);
      ...
      if (touch->GetVolume()->GetName() == "XTBL") {
         ...
         int merge_hit = 0;
         std::vector<GlueXHitECALblock::hitinfo_t>::iterator hiter;
         ...
         if (merge_hit) {
            ...
            // create new hit 
            hiter = block->hits.insert(hiter, GlueXHitECALblock::hitinfo_t());
            hiter->E_GeV = dEcorr/GeV;
            hiter->t_ns = tcorr/ns;
...
void GlueXSensitiveDetectorECAL::EndOfEvent(G4HCofThisEvent*)
{
   std::map<int,GlueXHitECALblock*> *blocks = fBlocksMap->GetMap();
   std::map<int,GlueXHitECALpoint*> *points = fPointsMap->GetMap();
   if (blocks->size() == 0 && points->size() == 0)
      return;
   std::map<int,GlueXHitECALblock*>::iterator biter;
   std::map<int,GlueXHitECALpoint*>::iterator piter;

   if (verboseLevel > 1) { 
      G4cout << G4endl
             << "--------> Hits Collection: in this event there are "
             << blocks->size() << " blocks with hits in the ECAL: "
             << G4endl;
      ...
      G4cout << G4endl
             << "--------> Hits Collection: in this event there are "
             << points->size() << " truth showers in the ECAL: "
             << G4endl;
   ...
   if (record == 0) {
      G4cerr << "GlueXSensitiveDetectorECAL::EndOfEvent error - "
   ...
   if (hitview.getCrystalEcals().size() == 0)
      hitview.addCrystalEcals();

   hddm_s::CrystalEcal &CrystalEcal = hitview.getCrystalEcal();

   // Collect and output the ecalTruthHits
   for (biter = blocks->begin(); biter != blocks->end(); ++biter) {
      std::vector<GlueXHitECALblock::hitinfo_t> &hits = biter->second->hits;
      ...
      if (hitscount > MAX_HITS) {
         hitscount = MAX_HITS;
         G4cerr << "GlueXSensitiveDetectorECAL::EndOfEvent warning:"
                << "max hit count " << MAX_HITS << " exceeded, "
                << hits.size() - hitscount << " hits discarded."
                << G4endl;
      }
      for (int ih=0; ih < hitscount; ++ih) {
         hddm_s::EcalTruthHitList thit = block(0).addEcalTruthHits(1);
         thit(0).setE(hits[ih].E_GeV);
         thit(0).setT(hits[ih].t_ns);
      }
   }

   // Collect and output the ecalTruthShowers
   for (piter = points->begin(); piter != points->end(); ++piter) {
      hddm_s::EcalTruthShowerList point = CrystalEcal.addEcalTruthShowers(1);
...
int GlueXSensitiveDetectorECAL::GetIdent(std::string div, 
